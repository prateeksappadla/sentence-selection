...
def bidirectional_rnn(forward_cell, backward_cell, inputs, seq_lens_mask, concatenate=True):
    ...
    seq_lens = tf.cast(tf.reduce_sum(seq_lens_mask, 1), tf.int64)
...
def rnn_decoder_attention(cell, start_state, inputs, inputs_mask, attentionf, attended, attended_mask):
    batch_size = tf.shape(inputs)[0]
    embedding_dim = inputs.get_shape().as_list()[2]
    context_time = tf.shape(attended)[1]
    ...
    # Continue with loop if condition returns true
    def condition(i, inputs, state, outputs, alpha_weights_time):
        return tf.less(i, time)

    # Body of while loop: runs one time step of RNN
    def body(i, inputs, state, outputs, alpha_weights_time):
        with tf.variable_scope("Cell-Time"):
            # Take one time step's worth of input and create mask (time_mask discussed in rnn_cell.py)
            input_ = tf.slice(inputs, [0, i, 0], [batch_size, 1, embedding_dim])
            ...
            alpha_weights, attend_result = \
                attentionf(attending=state, attended=attended, time_mask=attended_mask)
            # batch_size x context_time x 1
            alpha_weights_prime = tf.expand_dims(alpha_weights, -1)
            alpha_weights_time = tf.concat(2, [alpha_weights_time, alpha_weights_prime])
            ...
            outputs = tf.concat(1, [outputs, output])
        return [tf.add(i, 1), inputs, state, outputs, alpha_weights_time]
    ...
    # Did this because need to concatenate current time step's hidden state with all prev
        # timestep's hidden states; can't concatenate with "None" as one argument
    outputs_shape = tf.TensorShape([None, None, cell._state_size])
    # batch x context_time x query_time
    alpha_weights_shape = tf.TensorShape([None, None, None])
    initial_alpha = tf.zeros_like(tf.slice(attended, [0, 0, 0], [batch_size, -1, 1]))


    # Run RNN while loop
    _, _, last_state, hidden_states, alpha_weights = tf.while_loop(condition, body, \
        loop_vars=[i, inputs, state, tf.zeros([batch_size, 1, cell._state_size]), initial_alpha], \
        # Shape_invariants arg allows one to specify dimensions of inputs and outputs of each iterations
        # By using "None" as a dimension, signifies that this dimension can change
        shape_invariants=[i.get_shape(), inputs.get_shape(), state.get_shape(), outputs_shape, alpha_weights_shape])

    # get rid of zero output start state for concat purposes (see "body")
    hidden_states = tf.slice(hidden_states, [0, 1, 0], [batch_size, -1, cell._state_size])
    alpha_weights = tf.slice(alpha_weights, [0, 0, 1], [batch_size, -1, -1])

    # Dimensions: outputs (batch x time x hidden_size); last_state (batch x hidden_size)
    return (hidden_states, last_state, alpha_weights)
